...
%% to do: create dictionaries for this terms
...
%%%%%%%%%%%
    function out = read_string_data();
        ...
        aux{8,1} = 'X_Units';       % Physical units of that axis, like m or V.
        ...
        for i=1:size(aux,1)
            out.count(i) = fread(fid,1,'uint16');
            aux{i,2} = fread(fid,out.count(i),'uint16=>char')';
